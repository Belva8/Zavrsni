...
class KoraciBP  (context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun KreirajUnosKoraka(currentTimeMillis: Long): Boolean {
        var isDateAlreadyPresent = false
        var createSuccessful = false
        var currentDateStepCounts = 0
        val mCalendar = Calendar.getInstance()
        val todayDate = "${mCalendar.get(Calendar.MONTH) + 1}/${mCalendar.get(Calendar.DAY_OF_MONTH)}/${mCalendar.get(
            Calendar.YEAR)}"
    ...
    fun ProcitajUnosKoraka(): ArrayList<DatumKoraka> {
        val mListaKoraka = ArrayList<DatumKoraka>()
        ...
        try {
            ...
            if (c.moveToFirst()) {
                do {
                    val mDateStepsModel = DatumKoraka()
                    val dateIndex = c.getColumnIndex(CREATION_DATE)
                    val stepsIndex = c.getColumnIndex(STEPS_COUNT)
                    if (dateIndex != -1 && stepsIndex != -1) {
                        mDateStepsModel.mDatum = c.getString(dateIndex)
                        mDateStepsModel.mBrojKoraka = c.getInt(stepsIndex)
                        mListaKoraka.add(mDateStepsModel)
                    }
                } while (c.moveToNext())
            }
            db.close()
