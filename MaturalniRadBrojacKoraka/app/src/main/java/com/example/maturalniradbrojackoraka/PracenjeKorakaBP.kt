...
class PracenjeKorakaBP (context : Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun kreirajUnosKoraka(timeStamp: Long, stepType: Int, sessionId: String): Boolean {
        var createSuccessful = false
        val mCalendar = Calendar.getInstance()
        val todayDate =
            "${mCalendar.get(Calendar.MONTH) + 1}/${mCalendar.get(Calendar.DAY_OF_MONTH)}/${mCalendar.get(
                Calendar.YEAR)}"
        try {
    ...
    fun DohvatiKorakePoDatumu(date: String): IntArray {
        ...
        try {
            ...
            if (c != null && c.moveToFirst()) {
                ...
                if (stepTypeIndex != -1) {
                    ...
                    Log.e("PracenjeKorakaBP", "getColumnIndex returned -1 for STEP_TYPE column")
                }
            }
        } catch (e: Exception) {
            e.printStackTrace()
        } finally {
            // Close cursor and database in the finally block
            c?.close()
            db?.close()
        }
        return stepType
    ...
    @SuppressLint("Range")
    fun DohvatiUkupnoTrajanjeKoraka(): Long {
        var UkupnoTrajanje: Long = 0
        var db: SQLiteDatabase? = null
        var c: Cursor? = null
        try {
            ...
            if (c != null && c.moveToFirst()) {
                do {
                    ...
                    for (j in sizeStepTimeSessionList - 1 downTo 1) {
                        UkupnoTrajanje += (stepTimeSessionList[j] - stepTimeSessionList[j - 1])
                    }
                } while (c.moveToNext())
            }
        } catch (e: Exception) {
    ...
    @SuppressLint("Range")
    fun DohvatiDostupneDatume(): ArrayList<String> {
